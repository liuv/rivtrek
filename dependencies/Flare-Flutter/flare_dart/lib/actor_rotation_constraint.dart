...
class ActorRotationConstraint extends ActorTargetedConstraint {
  ...
  void constrain(ActorNode node) {
    ...
    ActorNode grandParent = node.parent!;

    Mat2D transformA = node.worldTransform;
    ...
    if (!_copy) {
      ...
      if (_offset) {
        _componentsB.rotation += node.rotation;
      }
    }
    ...
    bool clampLocal = _minMaxSpace == TransformSpace.Local;
    if (clampLocal) {
    ...
    Mat2D.compose(node.worldTransform, _componentsB);
  }
