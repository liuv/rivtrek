...
class _SharePreviewSheetState extends State<SharePreviewSheet> {
  ...
  void _loadPoi(String riverId, double km) {
    DatabaseService.instance.getNearestPoi(riverId, km).then((p) {
  ...
  Future<void> _captureAndShare() async {
    ...
    try {
      ...
      final file = File('${tempDir.path}/rivtrek_share_${DateTime.now().millisecondsSinceEpoch}.png');
      ...
      if (river != null) {
        final part = '我在涉川 Rivtrek 挑战「${river.name}」，已行至 ${challenge.currentDistance.toStringAsFixed(1)} km，当前河段：${challenge.currentSubSection?.name ?? "—"}';
        String? loc;
        if (_poi != null) {
          final parts = [_poi!.province, _poi!.city, _poi!.district, _poi!.township].whereType<String>().where((s) => s.isNotEmpty).toList();
          final addr = parts.isEmpty ? (_poi!.formattedAddress ?? '') : parts.join(' ');
          final names = _poi!.poisList.map((p) => p.name?.trim()).whereType<String>().where((s) => s.isNotEmpty).take(3).join('、');
          loc = addr.trim().isNotEmpty ? addr : null;
          if (names.isNotEmpty) loc = loc != null ? '$loc · $names' : names;
        }
        text = loc != null && loc.trim().isNotEmpty ? '$part，此刻行至 $loc。步履不停，丈量江山。' : '$part。步履不停，丈量江山。';
      }
  ...
  Widget build(BuildContext context) {
    ...
    final medalIconPath = sub?.medalIcon != null ? 'assets/${sub!.medalIcon}' : null;

    // 打开弹窗时按当前河流与里程拉取一次最近 POI
    if (_poi == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _loadPoi(river.id, challenge.currentDistance));
    }
    ...
    final locationLabel = _poi != null
        ? (addressParts.isEmpty ? (_poi!.formattedAddress ?? '') : addressParts.join(' '))
        : null;
